...
test_invoke (TSS2_SYS_CONTEXT *sys_context)
{
    ...
    *(UINT32 *)&(in_public.publicArea.objectAttributes) = 0;
    ...
    memcpy(input_plainText.buffer, message, input_plainText.size);
    in_scheme.scheme = TPM2_ALG_KDF1_SP800_56A;
    in_scheme.details.kdf1_sp800_56a.hashAlg = TPM2_ALG_SHA256;

    rc = Tss2_Sys_ECC_Encrypt(sys_context, loaded_sym_handle, 0, &input_message, &in_scheme, &c1, &c2, &c3, 0);
    if(rc != TPM2_RC_SUCCESS) {
        LOG_ERROR("ECC_Encrypt FAILED! Response Code : 0x%x", rc);
        exit(1);
    }
    ...
    if(rc != TPM2_RC_SUCCESS) {
        LOG_ERROR("ECC_Decrypt FAILED! Response Code : 0x%x", rc);
        exit(1);
    }
    LOG_INFO("ECC Decrypt successful.");

    /* Verify decrypted message matches original */
    if (output_message.size != input_message.size ||
        memcmp(output_message.buffer, input_message.buffer, output_message.size) != 0) {
