...
#include "tss2_esys.h"        // for ESYS_CONTEXT, ESYS_TR, Esys_ECC_Encrypt
 ...
 * @param[out] c1 First component of encrypted data.
 *             (callee-allocated)
 * @param[out] c2 Second component of encrypted data.
 *             (callee-allocated)
 * @param[out] c3 Third component of encrypted data.
 *             (callee-allocated)
    ...
    TPM2B_DIGEST **c3)
{
     ...
     */
    do {
        r = Esys_ECC_Encrypt_Finish(esysContext, c1, c2, c3);
...
{
    TSS2_RC r;
    LOG_TRACE("context=%p, keyHandle=%"PRIx32 ", plainText=%p,"
              "inScheme=%p",
              esysContext, keyHandle, plainText, inScheme);
    ...
    r = Tss2_Sys_ECC_Encrypt_Prepare(esysContext->sys,
                                     (keyHandleNode == NULL) ? TPM2_RH_NULL
                                      : keyHandleNode->rsrc.handle, plainText,
                                     inScheme);
...
{
    TSS2_RC r;
    LOG_TRACE("context=%p, c1=%p, c2=%p, c3=%p",
              esysContext, c1, c2, c3);
    ...
    if (c1 != NULL) {
        *c1 = calloc(1, sizeof(TPM2B_ECC_POINT));
        if (*c1 == NULL) {
            return_error(TSS2_ESYS_RC_MEMORY, "Out of memory");
        }
    }
    if (c2 != NULL) {
        *c2 = calloc(1, sizeof(TPM2B_MAX_BUFFER));
        if (*c2 == NULL) {
            return_error(TSS2_ESYS_RC_MEMORY, "Out of memory");
        }
    }
    if (c3 != NULL) {
        *c3 = calloc(1, sizeof(TPM2B_DIGEST));
        if (*c3 == NULL) {
            return_error(TSS2_ESYS_RC_MEMORY, "Out of memory");
        }
    }
